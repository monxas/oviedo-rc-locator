"""Validator HTTP routes — APIRouter mounted by service/validator_ui.py."""
import hmac
import json
import random
import time
...
from validator.env import TOKEN
from validator.labels import (
    _LABELS_LOCK,
    load_labels,
    save_labels,
    labeled_rcs,
...
def _random_rc() -> str:
    ...
    for k in keys:
        ...
        if cell is None:
            continue
        if f"{cell}-{sub}" not in _COVERED_CSUB:
            continue
        return rc
...
def _generate_for_rc(rc: str) -> dict:
    ...
    if not sheet:
        raise HTTPException(404, "RC sin hoja PGOU asociada")
...
@router.get("/api/img/{rc}/{kind}")
def img(rc: str, kind: str, _=Depends(auth)):
    data = _CACHE.get(rc)
    if not data:
        raise HTTPException(404, "cache miss — call /api/next first")
    if kind == "crop":
